...
namespace FlatFiles
{
    ...
    public interface INullFormatter
    {
        ...
        /// <summary>
        ///     Gets whether the given text should be interpreted as null, without the caller first copying it out of
        ///     the record it sits in. The default copies the text and inspects the string, so a formatter that can
        ///     answer from the characters alone should override it.
        /// </summary>
        /// <param name="context">The column context.</param>
        /// <param name="value">The value to inspect.</param>
        /// <returns>True if the value represents null; otherwise, false.</returns>
        bool IsNullValue( IColumnContext? context, ReadOnlySpan<char> value )
        {
            return IsNullValue( context, value.ToString() );
        }
