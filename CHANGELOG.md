...
The conditions are the reading path's, read the other way round: a column carrying an `OnFormatting` or `OnFormatted` hook, or a derived column that replaces `Format`, since all three are declared in terms of `object` or `string`; a custom mapping with its own writer; a nested entity; a member that is a field, has a non-public getter, or whose type is neither the column's nor its nullable form; an entity that is a value type; and a writer whose schema is chosen per record by an injector, since there are no values to choose one by until they have been worked out. Any of those and the write goes through the array as before.

`MappingAccessors` gained `AddGetter` and `AddNullableGetter`, so a caller without dynamic code can register the getters as they already register the setters. The method deliberately left out of 8.3.0's first release goes in here, where there is something for it to do.

One thing this turned up, and it is worth writing down because it cost the change most of its value until it was found: `FormatTyped` began life asking `if (value is null)` about a value of an unconstrained generic type, which the compiler answers **by boxing it**. Thirteen `int` members cost 387 bytes a record with the typed path supposedly avoiding boxes, against 77 for thirteen strings. Asking `!typeof( T ).IsValueType && value is null` instead folds to a constant where the type is a value type, and the test that would box is never reached. The same thirteen members then cost 75.

**A write fills one array for the whole write rather than one per record.** A typed write turned each entity into an `object[]` sized to the schema and let it go again, which for thirteen columns is 128 bytes a record spent on an array nothing keeps. The writer now fills one and reuses it. Measured on 10,000 records of 13 columns, a delimited write through a type mapper allocated 515 bytes a record and now allocates 387; fixed-length, 1,139 and now 1,011. That is the array in both cases, and it is the same change the reading side made in 8.1.0.

It is safer here than it was there. Nothing on the writing side is handed the values - there is no counterpart to the `RecordParsed` handler that has to be given an array of its own - so no record needs an exception made for it. What the reuse does need is clearing between records, because a mapping is not obliged to write every slot: a custom writer that leaves one alone would otherwise write the value the record before had put there. A test writes two records through such a mapping and fails at the second if the clearing goes.

A writer fed by an injector resizes instead, since consecutive records may belong to schemas of different widths.
