...
- **Bring the README up to date, and its Table of Contents with it.** The contents currently match the headings exactly, which is the easy half; the hard half is that two things a caller can reach have no heading at all. Constructor mapping shipped in 8.1.0 and is documented only in the changelog, so anybody wanting to map a positional record has to read release notes to discover they can. The integration test project is not mentioned either, and the root README is where somebody would look to find out how the library is held to account. Both want a section and an entry, and Conventions.md now says a release is not finished until they have one.
