...
In the order they will be built. Performance comes first here as it does everywhere else, so the item that measurement says is worth most leads. None of these breaks anything, so none of them is waiting for a major version.

- **Parse straight into the entity, without the `object?[]` and a box for every value.** A record is parsed into an array of boxed objects and the mapper then unboxes each one into a property. On 10,000 records of 13 columns that array and its boxes are most of what a read allocates: the same file read into ignored columns costs 123 bytes a record, into `Int32` columns 643, and into `string` columns 746. Removing the array copy above took 128 of it; the boxes are the other 312, and reaching them means the columns parsing to their own type and the generated deserialiser assigning it, rather than everything travelling as `object`.
