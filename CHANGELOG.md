...
One thing that used to work no longer does. The array reached through `IRecordContext.Values` is a copy of the record's values rather than the array the columns are parsed from, so writing to it from a parsing hook - `context.RecordContext.Values[1] = "99"` from an `OnParsed` on an earlier column - no longer changes what a later column parses. Substituting a value before the record is parsed is what `OnParsing`, the delimited reader's `RecordRead` handler and the fixed-length reader's `RecordPartitioned` handler are for, and all three still do it. The fixed-length reader has behaved this way since 7.5.0, where the change went unrecorded; this entry covers both readers.
