...
- The typed readers built by a type mapper selector threw `NullReferenceException` when no mapper matched a record and no default was set; they now throw `FlatFileException` naming the missing matcher. `FixedLengthReader`'s `IReader.GetSchemaAsync( CancellationToken )` also ignored its token; it now throws when the token is cancelled, as the typed overload did.
