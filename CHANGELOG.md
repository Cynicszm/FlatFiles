...
- **Constructor mapping, and reaching a property that only a constructor can set.** This entry used to say that a record, a type with `init` accessors and a type that validates in its constructor could none of them be mapped. Measured against the library rather than read off the signature, most of that is not so: `Define<T>()` does carry a `new()` constraint, but `Define( () => new Thing( ... ) )` does not, and through it a positional record, a type with `init` accessors and a type with `required` members all map and read correctly today, on the emitted path and the reflected one alike. What actually fails is narrower and worth stating properly.

  A property with no setter at all - set once by the constructor and get-only thereafter - cannot be written, and the reader says so: *Cannot read values into the Name property because it is read-only.* The only way to fill one is through the constructor, and the constructor is never given the parsed values: `UseFactory` takes no arguments, so a type that must be built complete has to be built with placeholders first and then contradicted, which a validating constructor will not allow.

  So the feature is constructor injection: parse the record, then pass the values to a constructor chosen by name or position, rather than constructing first and assigning after. That reaches get-only properties, lets a constructor validate what it is given, and removes the placeholder dance for records. It also lets `Define<T>()` drop its `new()` constraint for those types, which is the part a caller notices first, since today the constraint is a compile error rather than anything the library gets a chance to explain.
